...
- Package version: see `pubspec.yaml` (currently 2.15.0). Most architecture-significant changes ship as minor versions and are documented in `CHANGELOG.md`.
- SDK floor: Dart `>=3.6.0 <4.0.0`, Flutter `>=3.35.0`. (Raised in 2.15.0 to `3.6.0`/`3.27.0` to match reality — the color code uses the wide-gamut `Color` API `.r/.g/.b/.a` and `withValues`, both introduced in Flutter 3.27 / Dart 3.6; the older int accessors are deprecated and would fail `analyze --fatal-infos`. The Flutter half raised again, twice, in the unreleased version after 2.16.2 (task-012) once its CI `sdk-matrix` job started actually resolving `flutter pub get` at the declared floor instead of always running on `stable` (which had silently masked this): first to `3.29.0` (`flutter_streaming_text_markdown`, any version, needs `characters >=1.4.0`, unavailable in Flutter's bundled tooling below 3.29.0), then to `3.35.0` once the matrix job's SECOND run found the real binding constraint — `google_fonts ^8.1.0`'s own declared floor is `sdk ^3.9.0` / `flutter >=3.35.0`, higher than the `characters` one. The Dart `sdk:` constraint deliberately stayed at `3.6.0` throughout — Dart 3.7 gates a new default `dart format` style on the package's declared language version, so crossing that threshold would force-reformat the entire codebase (133 files) for no functional reason, and every Dart version any Flutter `>=3.35.0` actually ships already satisfies `>=3.6.0` anyway. This was the root cause of historical issues #7 and #12, and is exactly the kind of drift the `sdk-matrix` job now exists to catch automatically — including catching it in more than one dependency, one CI run at a time, since only an actual `pub get` at the floor surfaces the NEXT binding constraint once the first one is fixed.) Be careful before using newer language features, and don't change either bound (or bump a dependency's minimum version) without re-running the `sdk-matrix` job's floor leg to confirm the whole dependency tree still resolves — treat one green run as provisional, not proof there isn't a third constraint waiting behind it.
