...
### Fixed
- **The documented Flutter floor (`>=3.27.0`) was inaccurate — raised to `>=3.29.0`.** Found by task-012's new `sdk-matrix` CI job actually testing the declared floor for the first time (CI had always run on `stable`, silently masking this). `flutter_streaming_text_markdown` (every version, not just the currently-pinned `^1.9.1`) depends on `characters >=1.4.0`, but Flutter's own bundled `flutter_test` pins `characters` to exactly `1.3.0` through Flutter 3.27.4 — so `flutter pub get` has genuinely failed to resolve this package below Flutter 3.29.0 (the first stable release bundling `characters 1.4.0`) for as long as `flutter_streaming_text_markdown` has been a dependency. Anyone actually on Flutter 3.27.x-3.28.x was already unable to install this package; this only corrects the declared floor to match that pre-existing reality — it does not newly break anyone who could previously build. The Dart `sdk:` constraint stays at `>=3.6.0` (not bumped to Flutter 3.29.0's bundled `3.7.0`) deliberately, since Dart 3.7 gates a new default `dart format` style on the declared language version and every Dart version any Flutter `>=3.29.0` ships already satisfies `>=3.6.0` anyway.

### Added
- **Doc/code drift CI check + SDK matrix (task-012).** `tool/check_doc_drift.dart` scans every ```dart fence in README.md and AGENTS.md for `ClassName.member(...)` call sites and verifies each one resolves to a real factory constructor, named constructor, or static member actually declared on that class in `lib/` — the exact failure mode a past manual audit (`doc/ONBOARDING_AUDIT.md`) found by hand (a snippet calling a method/factory that no longer existed). Wired into CI (`analyze-format-test` job) so future drift is caught automatically. Also added a `sdk-matrix` CI job testing against both the documented consumer floor (Flutter 3.27.0 / Dart 3.6.0) and latest stable — the floor leg runs through `example/` rather than the root package directly, because the root's `dev_dependencies` (`flutter_lints: ^6.0.0`, needing Dart `^3.8.0` — see issue #41) are contributor-only tooling a real consumer never resolves; `example/` depends on this package the way a consumer actually would and resolves cleanly at the floor. As a side effect, `example/`'s own test suite (8 tests) now runs in CI for the first time.
