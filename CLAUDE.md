...
- SDK floor: Dart `>=3.6.0 <4.0.0`, Flutter `>=3.29.0`. (Raised in 2.15.0 to `3.6.0`/`3.27.0` to match reality — the color code uses the wide-gamut `Color` API `.r/.g/.b/.a` and `withValues`, both introduced in Flutter 3.27 / Dart 3.6; the older int accessors are deprecated and would fail `analyze --fatal-infos`. The Flutter half raised again in the unreleased version after 2.16.2 (task-012) to `3.29.0` — task-012's CI `sdk-matrix` job caught that the `3.27.0` floor was ALSO inaccurate for an unrelated reason: `flutter_streaming_text_markdown` (any version) depends on `characters >=1.4.0`, but Flutter's own bundled `flutter_test` pins `characters` to exactly `1.3.0` through 3.27.4 — so `flutter pub get` genuinely failed to resolve below Flutter 3.29.0 the whole time, silently, because CI always ran on `stable` and never tested the declared floor. The Dart `sdk:` constraint deliberately stayed at `3.6.0` rather than following Flutter 3.29.0's bundled `3.7.0` — Dart 3.7 gates a new default `dart format` style on the package's declared language version, so bumping it would force-reformat the entire codebase (133 files) for no functional reason, and every Dart version any Flutter `>=3.29.0` actually ships already satisfies `>=3.6.0` anyway. This was the root cause of historical issues #7 and #12, and is exactly the kind of drift the `sdk-matrix` job now exists to catch automatically.) Be careful before using newer language features, and don't change either bound without re-running the `sdk-matrix` job's floor leg to confirm the whole dependency tree actually resolves — the declared Flutter floor and the actual resolvable minimum have drifted apart twice now.
